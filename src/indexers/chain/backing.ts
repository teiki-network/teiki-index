...
export type Event = {
  ...
  flowerHashes: NonEmpty<string[]> | null;
};
...
export const setup = $.setup(async ({ sql }) => {
  ...
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS chain.flower (
      id bigint PRIMARY KEY REFERENCES chain.output (id) ON DELETE CASCADE,
...
export const filter = $.filter(
  ({
    ...
    if (backingIndicies.length) {
      return [
        { type: "backing", indicies: backingIndicies, flowerHashes: null },
      ];
    } else {
      ...
      if (minted) {
        ...
        for (const [asset, amount] of Object.entries(minted)) {
          ...
          for (const mph of mphProofOfBackings) {
            ...
            if (currentMph === mph) flowerHashes.push(tokenName);
          }
        }

        if (flowerHashes.length > 0)
          return [{ type: "backing", indicies: null, flowerHashes }];
      }
    }
    return null;
  }
    ...
    if (flowerHashes) {
      ...
      >`
        ...
        FROM
          ...
          INNER JOIN chain.flower f ON f.id = b.id
        WHERE
          (f.hash) IN ${sql(flowerHashes)}
          AND !f.is_matured
      `;
      for (const chainBacking of chainBackingWithOutputIds) {
        ...
        if (
          ...
          await sql`INSERT INTO chain.flower ${sql({
            id: chainBacking.id,
            slot,
            isMatured: true,
